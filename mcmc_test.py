...
csv file and rename the columns
    ...
    # saving csv
    df.to_csv(f'lat_{lat}_lon{lon}_radii{radii}.csv', index=False)
...
def plotter(sampler,days_simulate,flux):
    plt.ion()
    plt.plot(days,flux,label='test lightcurve')
    samples = sampler.flatchain
    for theta in samples[np.random.randint(len(samples), size=100)]:
        plt.plot(days, flux,_ =start_animate(theta_vec), color="r", alpha=0.1)
...
if __name__ == '__main__':
    ...
    # List of spots
    spots = []
    #parameters to running MCMC
    flux_err = 0.000005*np.mean(flux_test)
